		...
		string str_ylabel = "v_2", double sysd = -1.)
{
	...
	TH2D * hframe = new TH2D("hframe", ";p_{T};v_{2}", 1, 0, 8., 1, -0.1, 0.35);
	hframe->SetYTitle(str_ylabel.c_str());
	initHist(hframe);
	TH2D * hframeR = new TH2D("hframeR", ";p_{T};Ratio", 1, 0, 8., 1, 0.81, 1.19);
	initHist(hframeR);
...
//	grR2->Fit(fR2, "E", "Goff", 100, 300);
	TLine * lsys1 = new TLine(0, 1+sysd, 8., 1+sysd );
	TLine * lsys2 = new TLine(0, 1-sysd, 8., 1-sysd );
	if ( sysd > 0. ) {
		lsys1->Draw("same");
		lsys2->Draw("same");
	}
