...
int TMVAClassification_keras( string s = "LM" )
{
   ...
   TTree *signalTree     = (TTree*)inputS->Get("tree");
   TTree *background     = (TTree*)inputB->Get("tree");

   // Create a ROOT output file where TMVA will store ntuples, histograms, etc.
   TString outfileName( (string("TMVA_Keras_HydjetMCTruth_MCBkg_")+s+".root").c_str() ); // change
   ...
   TMVA::DataLoader *dataloader=new TMVA::DataLoader( (string("dataset_Keras_")+s).c_str() ); // change
   ...
   dataloader->AddVariable( "rapidity",         'F' );
   dataloader->AddVariable( "vtxChi2",          'F' );
   dataloader->AddVariable( "cosThetaXYZ",      'F' );
   dataloader->AddVariable( "Lxyz",             'F' );
   dataloader->AddVariable( "vtxDecaySigXYZ",   'F' );
...
//   TCut mycutb = "fabs(rapidity)<1.0"; // for example: TCut mycutb = "abs(var1)<0.5";
   ...
   {
      factory->BookMethod( dataloader, TMVA::Types::kBDT, "BDTG1000_D2",
                           "H:!V:NTrees=1000:MinNodeSize=2.5%:BoostType=Grad:Shrinkage=0.10:UseBaggedBoost:BaggedSampleFraction=0.5:nCuts=20:MaxDepth=2" );
      factory->BookMethod( dataloader, TMVA::Types::kBDT, "BDTG250_D2",
                           "H:!V:NTrees=250:MinNodeSize=2.5%:BoostType=Grad:Shrinkage=0.10:UseBaggedBoost:BaggedSampleFraction=0.5:nCuts=20:MaxDepth=2" );
      factory->BookMethod( dataloader, TMVA::Types::kBDT, "BDTG250_D3",
                           "H:!V:NTrees=250:MinNodeSize=2.5%:BoostType=Grad:Shrinkage=0.10:UseBaggedBoost:BaggedSampleFraction=0.5:nCuts=20:MaxDepth=3" );
   }
...
//                           "!H:!V:NTrees=250:MinNodeSize=2.5%:MaxDepth=2:BoostType=AdaBoost:AdaBoostBeta=0.5:UseBaggedBoost:BaggedSampleFraction=0.5:SeparationType=GiniIndex:nCuts=20" );
   ...
   // Keras TMVA
   if (Use["Keras"]) {
      factory->BookMethod( dataloader, TMVA::Types::kPyKeras, "Keras128Sparse",
              "H:!V:FilenameModel=model_128sparse.h5:NumEpochs=20");
      factory->BookMethod( dataloader, TMVA::Types::kPyKeras, "Keras128SparseDG",
              "H:!V:VarTransform=D,G:FilenameModel=model_128sparse.h5:NumEpochs=20:BatchSize=32");
   }
