...
void MCsignificanceCent(string s = "LM", int BDT = 15, int centS = 0 )
{
    bool bReweight = true;
    TTree * testTree = 0;
    TH1D * hmass_Signal = 0;
    ...
    TFile * f = new TFile( (string("TMVA3_HydjetMCTruth_MCBkg_") + s + "_" + to_string(centS) + ".root").c_str() );
    testTree = (TTree*) f->Get( (string("dataset3_full_") + s + "_" + to_string(centS) + "/TestTree").c_str() );
    if ( s == "LM" ) {
    ...
    TFile * fsave = new TFile( (string("MCsignificanceCent/") + s + to_string(centS) + "_" + to_string(BDT) + ".root").c_str(), "recreate" );
    hmass_Signal->Write();
    hmass_Bckgnd->Write();
