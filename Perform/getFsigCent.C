...
void getFsigCent(string s = "LM", string y = "Mid", string option = "")
{
    ...
    for ( int c = 1; c < 5; c++ ) {
        for ( int ipt = 4; ipt < 13; ipt++ ) {
            TFile f( (s + option + "/hMass" + y + "_" + to_string(c) + "_" + to_string(ipt) + "_centS.root" ).c_str() );
    ...
    TFile * fsave = new TFile((string("Fsig") + s + option + y + "_centS.root").c_str(), "recreate");
    for ( int c = 0; c < 6; c++ ) {
        hFsig[c]->Write();
    }
