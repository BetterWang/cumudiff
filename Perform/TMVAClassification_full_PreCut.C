...
int TMVAClassification_full_PreCut( string s = "LM" )
{
   ...
   TString outfileName( (string("TMVA3PreCut_HydjetMCTruth_MCBkg_")+s+".root").c_str() ); // change
   ...
   TMVA::DataLoader *dataloader=new TMVA::DataLoader( (string("dataset3PreCut_full_")+s).c_str() ); // change
...
//   TCut mycutb = "fabs(rapidity)<1.0"; // for example: TCut mycutb = "abs(var1)<0.5";
   ...
   // LM only
   TCut mycuts = "pTrkDCASigXY<60 && pTrkDCASigZ<60 && nTrkDCASigXY<60 && nTrkDCASigZ<60 && Lxyz<50";
   TCut mycutb = "pTrkDCASigXY<60 && pTrkDCASigZ<60 && nTrkDCASigXY<60 && nTrkDCASigZ<60 && Lxyz<50";
